...
export default function WebampPlayerApp({ initialFile, onClose }: WebampPlayerAppProps) {
  ...
  return (
    <div className="w-full h-full bg-[#0d0e12] text-white flex flex-col justify-between items-center relative overflow-hidden font-sans select-none">
      <input
        type="file"
        ref={fileInputRef}
      ...
      {/* Control Bar Header */}
      <div className="w-full bg-[#18181c] border-b border-white/10 px-3 py-2 flex items-center justify-between gap-2 z-40 shrink-0">
        <div className="flex items-center gap-2">
          <div className="p-1.5 bg-amber-500/20 text-amber-400 rounded-lg border border-amber-500/30">
            <Disc className="w-4 h-4 animate-spin-slow" />
          </div>
          <div>
            <h2 className="text-xs font-bold text-white tracking-wide">Webamp 2.91 Core</h2>
            <p className="text-[10px] text-gray-400 font-mono">Reproductor Digital de Audio & VFS</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-2.5 py-1 bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer shadow-sm active:scale-95"
            title="Añadir archivos MP3 locales desde su PC"
          >
            <Upload className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">+ MP3 Local</span>
          </button>

          {/* Button: Browse VFS */}
          <button
            onClick={() => setIsVfsOpen(true)}
            className="px-2.5 py-1 bg-sky-500/20 hover:bg-sky-500/30 text-sky-300 border border-sky-500/40 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer shadow-sm active:scale-95"
            title="Explorar y cargar archivos MP3 alojados en el sistema de archivos virtual VFS"
          >
            <Folder className="w-3.5 h-3.5 text-sky-400" />
            <span>📁 VFS SaviaOS</span>
          </button>
        </div>
      </div>

      {/* Main Webamp Canvas Container */}
      <div className="w-full flex-1 flex items-center justify-center p-2 relative overflow-auto custom-scrollbar">
        {errorMsg ? (
          <div className="text-rose-400 text-xs font-semibold">{errorMsg}</div>
        ) : (
